	...
	public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
		if (buttonView.equals(mIgnoreButton)) {
			if (isChecked && mReactButton.isChecked()) {
				...
				mListener.onDuelOptionsSelected(Constants.MODE_REACT_CHAIN_OPTION, false);
			}
			mListener.onDuelOptionsSelected(Constants.MODE_IGNORE_CHAIN_OPTION, isChecked);
		} else {
			if (isChecked && mIgnoreButton.isChecked()) {
				mIgnoreButton.setChecked(false);
				mListener.onDuelOptionsSelected(Constants.MODE_IGNORE_CHAIN_OPTION, false);
			}
			mListener.onDuelOptionsSelected(Constants.MODE_REACT_CHAIN_OPTION, isChecked);
		}		
	}
