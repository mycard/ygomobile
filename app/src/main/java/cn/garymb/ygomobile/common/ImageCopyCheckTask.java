...
public class ImageCopyCheckTask extends AsyncTask<Object, Boolean, Bundle> {
	...
	private ImageCheckListener mListener;
	
	private Context mContext;

	public interface ImageCheckListener {
		void onCheckComplete(Bundle result);
	}
	...
	public ImageCopyCheckTask(Context context) {
		mContext = context;
	...
	public void setImageDLCheckListener(ImageCheckListener listener) {
		mListener = listener;
	}
	...
	protected Bundle doInBackground(Object... params) {
		Bundle bundle = null;
		File imageDir = new File(StaticApplication.get()
				.getCardImagePath());
		...
		try {
			...
			if (cursor != null && cursor.moveToFirst()) {
				...
				for (Integer id : ids) {
					try {
						FileOpsUtils.assetsCopy(mContext, "pics/"+ id+ ".jpg", ImageItemInfoHelper.getImagePath(id + ""), false);
					} catch (IOException e) {
						e.printStackTrace();
						continue;
					}
	...
	protected void onPostExecute(Bundle result) {
		...
		mWaitDialog = null;
		if (mListener != null) {
			mListener.onCheckComplete(result);
			mListener = null;
		}
	}
