...
public class DatabaseUtils {
    ...
    public static boolean checkAndCopyFromInternalDatabase(Context context, String path,
        ...
        if (!checkDataBase(context, path, needsUpdate)) {
            Log.i("checker", "check cdb");
            try {
                new File(path).mkdirs();
                String file = new File(path, YGOCardsProvider.DATABASE_NAME).getAbsolutePath();
                FileOpsUtils.assetsCopy(context, YGOCardsProvider.DATABASE_NAME, file, true);
                doSomeTrickOnDatabase(file);
            } catch (IOException e) {
                result = false;
                e.printStackTrace();
            } catch (SQLiteException e) {
    ...
    public static void doSomeTrickOnDatabase(String myPath)
        ...
        try {
            Log.i("checker", "doSomeTrickOnDatabase");
            db.beginTransaction();
            ...
            db.execSQL("CREATE TABLE texts (_id integer PRIMARY KEY, name varchar(128), desc varchar(2048),"
            ...
            Log.e("checker", "doSomeTrickOnDatabase",e);
        }finally {
            db.endTransaction();
            db.close();
        }
