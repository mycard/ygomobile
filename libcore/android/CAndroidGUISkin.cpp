...
CAndroidGUISkin* CAndroidGUISkin::createAndroidSkin(EGUI_SKIN_TYPE type,
		video::IVideoDriver* driver, IGUIEnvironment* env, float xScale, float yScale) {
	CAndroidGUISkin* skin = new CAndroidGUISkin(type, driver, xScale, xScale);
...
CAndroidGUISkin::CAndroidGUISkin(EGUI_SKIN_TYPE type,
		video::IVideoDriver* driver, float xScale, float yScale) :
		SpriteBank(0), Driver(driver), Type(type), XScale(xScale), YScale(yScale) {
#ifdef _DEBUG
...
CAndroidGUISkin::~CAndroidGUISkin() {
	for (u32 i = 0; i < EGDF_COUNT; ++i) {
		if (Fonts[i])
			Fonts[i]->drop();
	}
...
video::SColor CAndroidGUISkin::getColor(EGUI_DEFAULT_COLOR color) const {
...
void CAndroidGUISkin::setColor(EGUI_DEFAULT_COLOR which,
...
s32 CAndroidGUISkin::getSize(EGUI_DEFAULT_SIZE size) const {
...
//! sets a default size
void CAndroidGUISkin::setSize(EGUI_DEFAULT_SIZE which, s32 size) {
	if ((u32) which < EGDS_COUNT)
		Sizes[which] = size;
}

//! returns the default font
IGUIFont* CAndroidGUISkin::getFont(EGUI_DEFAULT_FONT which) const {
...
//! sets a default font
void CAndroidGUISkin::setFont(IGUIFont* font, EGUI_DEFAULT_FONT which) {
...
IGUISpriteBank* CAndroidGUISkin::getSpriteBank() const {
	return SpriteBank;
}

//! set a new sprite bank or remove one by passing 0
void CAndroidGUISkin::setSpriteBank(IGUISpriteBank* bank) {
	if (bank)
		bank->grab();
...
u32 CAndroidGUISkin::getIcon(EGUI_DEFAULT_ICON icon) const {
...
//! Sets a default icon
void CAndroidGUISkin::setIcon(EGUI_DEFAULT_ICON icon, u32 index) {
	if ((u32) icon < EGDI_COUNT)
		Icons[icon] = index;
}
...
const wchar_t* CAndroidGUISkin::getDefaultText(EGUI_DEFAULT_TEXT text) const {
...
//! "OK", "Cancel", "Yes", "No" and so on.
void CAndroidGUISkin::setDefaultText(EGUI_DEFAULT_TEXT which,
...
void CAndroidGUISkin::draw3DButtonPaneStandard(IGUIElement* element,
...
/**	Used for drawing for example buttons in pressed state.
 ...
 is usually not used by ISkin, but can be used for example by more complex
 implementations to find out how to draw the part exactly. */
void CAndroidGUISkin::draw3DButtonPanePressed(IGUIElement* element,
		const core::rect<s32>& r, const core::rect<s32>* clip) {
	if (!Driver)
		return;
...
/** Used for drawing the background of edit, combo or check boxes.
 ...
 \param rect: Defining area where to draw.
 \param clip: Clip area.	*/
void CAndroidGUISkin::draw3DSunkenPane(IGUIElement* element,
...
core::rect<s32> CAndroidGUISkin::draw3DWindowBackground(IGUIElement* element,
	...
	if (!checkClientArea) {
		if (!UseGradient) {
			...
			const video::SColor c1 = getColor(EGDC_WINDOW).getInterpolated(
					0xFFFFFFFF, 0.9f);
			const video::SColor c2 = getColor(EGDC_WINDOW).getInterpolated(
					0xFFFFFFFF, 0.8f);
		...
		if (checkClientArea) {
			...
			if (Type == EGST_BURNING_SKIN) {
				const video::SColor c = titleBarColor.getInterpolated(
						video::SColor(titleBarColor.getAlpha(), 255, 255, 255),
						0.8f);
				Driver->draw2DRectangle(rect, titleBarColor, titleBarColor, c,
						c, clip);
			} else {
				const video::SColor c = titleBarColor.getInterpolated(
						video::SColor(titleBarColor.getAlpha(), 0, 0, 0), 0.2f);
				Driver->draw2DRectangle(rect, titleBarColor, c, titleBarColor,
						c, clip);
			}
		}
	}
...
/**	Used for drawing for menus and context menus.
 ...
 \param rect: Defining area where to draw.
 \param clip: Clip area.	*/
void CAndroidGUISkin::draw3DMenuPane(IGUIElement* element,
		const core::rect<s32>& r, const core::rect<s32>* clip) {
	if (!Driver)
		return;
...
/**	Used for drawing for toolbars and menus.
 ...
 \param rect: Defining area where to draw.
 \param clip: Clip area.	*/
void CAndroidGUISkin::draw3DToolBar(IGUIElement* element,
		const core::rect<s32>& r, const core::rect<s32>* clip) {
	if (!Driver)
		return;
...
/**	Used for drawing for tab buttons on top of tabs.
 ...
 \param rect: Defining area where to draw.
 \param clip: Clip area.	*/
void CAndroidGUISkin::draw3DTabButton(IGUIElement* element, bool active,
		const core::rect<s32>& frameRect, const core::rect<s32>* clip,
		EGUI_ALIGNMENT alignment) {
	if (!Driver)
		return;
...
/**	\param element: Pointer to the element which wishes to draw this. This parameter
 ...
 \param rect: Defining area where to draw.
 \param clip: Clip area.	*/
void CAndroidGUISkin::draw3DTabBody(IGUIElement* element, bool border,
		bool background, const core::rect<s32>& rect,
		const core::rect<s32>* clip, s32 tabHeight, EGUI_ALIGNMENT alignment) {
	if (!Driver)
		return;
...
/**	\param parent: Pointer to the element which wishes to draw this icon.
 ...
 \param clip: Clip area.	*/
void CAndroidGUISkin::drawIcon(IGUIElement* element, EGUI_DEFAULT_ICON icon,
...
EGUI_SKIN_TYPE CAndroidGUISkin::getType() const {
	return Type;
}

//! draws a 2d rectangle.
void CAndroidGUISkin::draw2DRectangle(IGUIElement* element,
...
void CAndroidGUISkin::serializeAttributes(io::IAttributes* out,
...
void CAndroidGUISkin::deserializeAttributes(io::IAttributes* in,
