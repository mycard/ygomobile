	...
	if(showcard) {
		...
		case 101: {
			...
			if(showcardp < 10) {
				...
				recti loc = recti(650 * mainGame->xScale - pos.Width / 2, 290 * mainGame->yScale, 950 * mainGame->xScale, 370 * mainGame->yScale);
...
void Game::DrawBackImage(irr::video::ITexture* texture) {
	if(!texture)
		return;
	driver->draw2DImage(texture, recti(0, 0, 1280 * mainGame->xScale, 720 * mainGame->yScale), recti(0, 0, texture->getOriginalSize().Width, texture->getOriginalSize().Height));
}
...
void Game::DrawThumb(code_pointer cp, position2di pos, std::unordered_map<int, int>* lflist) {
	...
	if(mainGame->cbLimit->getSelected() >= 4) {
		switch(cp->second.ot) {
		case 1:
			driver->draw2DImage(imageManager.tOT, recti(pos.X + 7 * mainGame->xScale, pos.Y + 50 * mainGame->yScale, pos.X + 37 * mainGame->xScale, pos.Y + 65 * mainGame->yScale), recti(0, 0, 128, 64), 0, 0, true);
			break;
		case 2:
			driver->draw2DImage(imageManager.tOT, recti(pos.X + 7 * mainGame->xScale, pos.Y + 50 * mainGame->yScale, pos.X + 37 * mainGame->xScale, pos.Y + 65 * mainGame->yScale), recti(0, 64, 128, 128), 0, 0, true);
			break;
		}
	}
}
void Game::DrawDeckBd() {
	...
	for(size_t i = 0; i < 7 && i + mainGame->scrFilter->getPos() < deckBuilder.results.size(); ++i) {
		...
		if(ptr->second.type & TYPE_MONSTER) {
			...
			textFont->draw(textBuffer, recti(904 * mainGame->xScale, (187 + i * 66) * mainGame->yScale, 1000 * mainGame->xScale, (207 + i * 66) * mainGame->yScale), 0xffffffff, false, false);
			if(ptr->second.attack < 0 && ptr->second.defense < 0)
				...
				textFont->draw(textBuffer, recti(904 * mainGame->xScale, (186 + i * 66) * mainGame->yScale, 955 * mainGame->xScale, (207 + i * 66) * mainGame->yScale), 0xff000000, false, false);
				textFont->draw(textBuffer, recti(904 * mainGame->xScale, (187 + i * 66) * mainGame->yScale, 955 * mainGame->xScale, (207 + i * 66) * mainGame->yScale), 0xffffffff, false, false);
