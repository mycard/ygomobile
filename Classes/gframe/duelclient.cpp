...
void DuelClient::HandleSTOCPacketLan(char* data, unsigned int len) {
	...
	case STOC_CHANGE_SIDE: {
		mainGame->soundEffectPlayer->doDeckBgm();
		mainGame->gMutex.Lock();
		mainGame->dInfo.isStarted = false;
...
int DuelClient::ClientAnalyze(char * msg, unsigned int len) {
	...
	case MSG_WIN: {
		...
		if(player == 2) {
			...
			mainGame->soundEffectPlayer->doLoseBgm(); 
		}else if(mainGame->LocalPlayer(player) == 0) {
			mainGame->showcardcode = 1;
			mainGame->soundEffectPlayer->doWinBgm();
			if(match_kill)
	...
	case MSG_START: {
		...
		mainGame->gMutex.Unlock();
		mainGame->soundEffectPlayer->doOpeningBGM();
		return true;
	}
	...
	case MSG_DRAW: {
		...
		for (int i = 0; i < count; ++i) {
			unsigned int code = (unsigned int)BufferIO::ReadInt32(pbuf);
			pcard = mainGame->dField.GetCard(player, LOCATION_DECK,
			mainGame->dField.deck[player].size() - 1 - i);
		...
		if(mainGame->dInfo.isReplay && mainGame->dInfo.isReplaySkiping) {
			for (int i = 0; i < count; ++i) {
				pcard = mainGame->dField.GetCard(player, LOCATION_DECK,
						mainGame->dField.deck[player].size() - 1);
				mainGame->dField.deck[player].erase(
						mainGame->dField.deck[player].end() - 1);
			...
			for (int i = 0; i < count; ++i) {
				...
				mainGame->dField.deck[player].erase(
						mainGame->dField.deck[player].end() - 1);
				mainGame->dField.AddCard(pcard, player, LOCATION_HAND, 0);
				for (size_t i = 0; i < mainGame->dField.hand[player].size();
						++i)
					mainGame->dField.MoveCard(mainGame->dField.hand[player][i],
							10);
				mainGame->gMutex.Unlock();
				mainGame->WaitFrameSignal(5);
			}
	...
	case MSG_DAMAGE: {
		...
		if(mainGame->dInfo.isReplay && mainGame->dInfo.isReplaySkiping) {
			...
			myswprintf(mainGame->dInfo.strLP[player], L"%d",
			mainGame->dInfo.lp[player]);
			return true;
		}
