...
bool DeckBuilder::OnEvent(const irr::SEvent& event) {
	...
	case irr::EET_MOUSE_INPUT_EVENT: {
		...
		case irr::EMIE_MOUSE_MOVED: {
			...
			if(x >= 314 * mainGame->xScale && x <= 794 * mainGame->xScale && y >= 164 * mainGame->yScale && y <= 435 * mainGame->yScale) {
				...
				if(x >= 750 * mainGame->xScale)
					px = lx - 1;
				else px = (x - 314 * mainGame->xScale) * (lx - 1) / (436 * mainGame->xScale);
			...
			} else if(x >= 314 * mainGame->xScale && x <= 794 * mainGame->xScale && y >= 466 * mainGame->yScale && y <= 530 * mainGame->yScale) {
				...
				if(x >= 750 * mainGame->xScale)
					hovered_seq = lx - 1;
				else hovered_seq = (x - 314 * mainGame->xScale) * (lx - 1) / (436 * mainGame->xScale);
			...
			} else if (x >= 314 * mainGame->xScale && x <= 794 * mainGame->xScale && y >= 564 * mainGame->yScale && y <= 628 * mainGame->yScale) {
...
#ifdef _IRR_ANDROID_PLATFORM_
			} else if(x >= 855 * mainGame->xScale && x <= 995 * mainGame->xScale && y >= 165 * mainGame->yScale && y <= 626 * mainGame->yScale) {
#else
