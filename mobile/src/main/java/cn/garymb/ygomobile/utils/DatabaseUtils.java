...
public class DatabaseUtils {
	...
	private static boolean checkDataBase(Context context, String path,
			boolean needsUpdate) {
		if (needsUpdate) {
			new File(path, GameSettings.DATABASE_NAME).delete();
			return false;
		}
		return new File(path, GameSettings.DATABASE_NAME).exists();
	}
	...
	public static boolean checkAndCopyFromExternalDatabase(Context context, String fromPath, String toPath,
		...
		if (!checkDataBase(context, toPath, needsUpdate)) {
			try {
				...
				FileOpsUtils.copyFileUsingFileChannels(new File(fromPath), new File(toPath, GameSettings.DATABASE_NAME));
	...
	public static boolean checkAndCopyFromInternalDatabase(Context context, String path,
		...
		if (!checkDataBase(context, path, needsUpdate)) {
			try {
				...
				FileOpsUtils.assetsCopy(context, "cards.cdb",new File(path, GameSettings.DATABASE_NAME).getAbsolutePath(), true);
	...
	private static void doSomeTrickOnDatabase(String path)
		...
		String myPath = path + File.separator + GameSettings.DATABASE_NAME;
